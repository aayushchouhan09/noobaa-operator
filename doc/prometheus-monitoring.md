...
NooBaa exposes Prometheus metrics for monitoring system health, web server activity, and background workers. This document focuses on containerized deployments and highlights how to access metrics endpoints, scrape them with Prometheus, and verify outputs.
...
- If your Prometheus instance uses label selectors, ensure it matches the ServiceMonitor labels (default: `app: noobaa`).
...
The web_server, bg_workers, and hosted_agents endpoints export a large and evolving set of runtime metrics. Ensure the token is set (see [Set JWT Token](#3-set-jwt-token)) and use either the port-forward from step 4 or exec from step 5, then run these queries:
...
#### 3. Set JWT Token
Export the bearer token once and reuse it in the following steps.
...
```sh
# Keep these port-forward commands running in separate terminals.
...
<a id="prometheus-ui"></a>
#### 6. Local Prometheus dashboard testing </br>
...
- Prometheus UI (see [Local Prometheus dashboard testing](#prometheus-ui)): `http://127.0.0.1:9090/alerts` and `http://127.0.0.1:9090/rules`
